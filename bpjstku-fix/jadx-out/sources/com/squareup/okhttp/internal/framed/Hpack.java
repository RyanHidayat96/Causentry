...
final class Hpack {
    ...
    private static final Header[] STATIC_HEADER_TABLE = {new Header(Header.TARGET_AUTHORITY, ""), new Header(Header.TARGET_METHOD, ShareTarget.METHOD_GET), new Header(Header.TARGET_METHOD, ShareTarget.METHOD_POST), new Header(Header.TARGET_PATH, "/"), new Header(Header.TARGET_PATH, "/index.html"), new Header(Header.TARGET_SCHEME, "http"), new Header(Header.TARGET_SCHEME, "https"), new Header(Header.RESPONSE_STATUS, Constants.STATUS_CODE_200), new Header(Header.RESPONSE_STATUS, "204"), new Header(Header.RESPONSE_STATUS, "206"), new Header(Header.RESPONSE_STATUS, "304"), new Header(Header.RESPONSE_STATUS, Constants.STATUS_CODE_400), new Header(Header.RESPONSE_STATUS, "404"), new Header(Header.RESPONSE_STATUS, "500"), new Header("accept-charset", ""), new Header("accept-encoding", "gzip, deflate"), new Header("accept-language", ""), new Header("accept-ranges", ""), new Header("accept", ""), new Header("access-control-allow-origin", ""), new Header("age", ""), new Header("allow", ""), new Header("authorization", ""), new Header("cache-control", ""), new Header("content-disposition", ""), new Header("content-encoding", ""), new Header("content-language", ""), new Header("content-length", ""), new Header("content-location", ""), new Header("content-range", ""), new Header("content-type", ""), new Header("cookie", ""), new Header("date", ""), new Header("etag", ""), new Header("expect", ""), new Header("expires", ""), new Header("from", ""), new Header("host", ""), new Header("if-match", ""), new Header("if-modified-since", ""), new Header("if-none-match", ""), new Header("if-range", ""), new Header("if-unmodified-since", ""), new Header("last-modified", ""), new Header(DynamicLink.Builder.KEY_LINK, ""), new Header(FirebaseAnalytics.Param.LOCATION, ""), new Header("max-forwards", ""), new Header("proxy-authenticate", ""), new Header("proxy-authorization", ""), new Header("range", ""), new Header("referer", ""), new Header("refresh", ""), new Header("retry-after", ""), new Header("server", ""), new Header("set-cookie", ""), new Header("strict-transport-security", ""), new Header("transfer-encoding", ""), new Header("user-agent", ""), new Header("vary", ""), new Header("via", ""), new Header("www-authenticate", "")};
    ...
    static final class Reader {
        ...
        Reader(int i, tryToComplete trytocomplete) {
        ...
        final void readHeaders() throws IOException {
            while (!this.source.d()) {
                ...
                int i = bCancel & UByte.MAX_VALUE;
                if (i == 128) {
                    throw new IOException("index == 0");
                }
                if ((bCancel & ByteCompanionObject.MIN_VALUE) == 128) {
                    readIndexedHeader(readInt(i, 127) - 1);
                } else if (i == 64) {
                    readLiteralHeaderWithIncrementalIndexingNewName();
                } else if ((bCancel & 64) == 64) {
                    readLiteralHeaderWithIncrementalIndexingIndexedName(readInt(i, 63) - 1);
                } else if ((bCancel & 32) == 32) {
                    int i2 = readInt(i, 31);
        ...
        private int readByte() throws IOException {
            return this.source.cancel() & UByte.MAX_VALUE;
        }
        ...
        final ByteString readByteString() throws IOException {
            ...
            if (z) {
                return ByteString.TuitionPaymentFragmentbindingInflater1(Huffman.get().decode(this.source.b(i2)));
            }
            return this.source.TuitionPaymentFragmentbindingInflater1(i2);
        }
    }
    ...
    static final class Writer {
        ...
        Writer(RotationProviderListenerWrapper rotationProviderListenerWrapper) {
        ...
        final void writeHeaders(List<Header> list) throws IOException {
            ...
            for (int i = 0; i < size; i++) {
                ByteString byteStringG = list.get(i).name.g();
                Integer num = (Integer) Hpack.NAME_TO_FIRST_INDEX.get(byteStringG);
                if (num != null) {
                    writeInt(num.intValue() + 1, 15, 0);
                    writeByteString(list.get(i).value);
                } else {
                    this.out.TuitionPaymentFragmentspecialinlinedviewModeldefault3(0);
        ...
        final void writeInt(int i, int i2, int i3) throws IOException {
            if (i < i2) {
                this.out.TuitionPaymentFragmentspecialinlinedviewModeldefault3(i | i3);
                return;
            }
            this.out.TuitionPaymentFragmentspecialinlinedviewModeldefault3(i3 | i2);
            int i4 = i - i2;
            while (i4 >= 128) {
                this.out.TuitionPaymentFragmentspecialinlinedviewModeldefault3(128 | (i4 & 127));
                i4 >>>= 7;
            }
            this.out.TuitionPaymentFragmentspecialinlinedviewModeldefault3(i4);
        }

        final void writeByteString(ByteString byteString) throws IOException {
            writeInt(byteString.TuitionPaymentFragmentspecialinlinedviewModeldefault3(), 127, 0);
            RotationProviderListenerWrapper rotationProviderListenerWrapper = this.out;
