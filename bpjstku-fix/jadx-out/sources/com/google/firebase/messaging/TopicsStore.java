...
final class TopicsStore {
    ...
    final TopicOperation getNextTopicOperation() {
        TopicOperation topicOperationFrom;
        synchronized (this) {
            topicOperationFrom = TopicOperation.from(this.topicOperationsQueue.peek());
        }
        return topicOperationFrom;
    }

    final boolean addTopicOperation(TopicOperation topicOperation) {
        boolean zAdd;
        synchronized (this) {
            zAdd = this.topicOperationsQueue.add(topicOperation.serialize());
        }
        return zAdd;
    }

    final boolean removeTopicOperation(TopicOperation topicOperation) {
        boolean zRemove;
        synchronized (this) {
            zRemove = this.topicOperationsQueue.remove(topicOperation.serialize());
        }
        return zRemove;
    }
    ...
    final TopicOperation pollTopicOperation() {
        ...
        synchronized (this) {
            try {
                topicOperationFrom = TopicOperation.from(this.topicOperationsQueue.remove());
