...
public class MessagingAnalytics {
    ...
    static boolean deliveryMetricsExportToBigQueryEnabled() {
        ...
        try {
            ...
            if (packageManager != null && (applicationInfo = packageManager.getApplicationInfo(applicationContext.getPackageName(), 128)) != null && ((PackageItemInfo) applicationInfo).metaData != null && ((PackageItemInfo) applicationInfo).metaData.containsKey(MANIFEST_DELIVERY_METRICS_EXPORT_TO_BIG_QUERY_ENABLED)) {
                return ((PackageItemInfo) applicationInfo).metaData.getBoolean(MANIFEST_DELIVERY_METRICS_EXPORT_TO_BIG_QUERY_ENABLED, false);
            }
            return false;
    ...
    private static void logToFirelog(MessagingClientEvent.Event event, Intent intent, getCaptureFuture getcapturefuture) {
        MessagingClientEvent messagingClientEventEventToProto;
        if (getcapturefuture == null || (messagingClientEventEventToProto = eventToProto(event, intent)) == null) {
            return;
        }
        try {
            getcapturefuture.b(Constants.FirelogAnalytics.FCM_LOG_SOURCE, MessagingClientEventExtension.class, new ProcessingNodeExternalSyntheticLambda7("proto"), new ProcessingRequest() { // from class: com.google.firebase.messaging.MessagingAnalytics$$ExternalSyntheticLambda0
                @Override // defpackage.ProcessingRequest
                public final Object apply(Object obj) {
                    return ((MessagingClientEventExtension) obj).toByteArray();
                }
            }).TuitionPaymentFragmentspecialinlinedviewModeldefault2(new ProcessingNodeExternalSyntheticLambda0(MessagingClientEventExtension.newBuilder().setMessagingClientEvent(messagingClientEventEventToProto).build(), Priority.DEFAULT, new ProcessingNodeInputPacket(Integer.valueOf(intent.getIntExtra(Constants.MessagePayloadKeys.PRODUCT_ID, DEFAULT_PRODUCT_ID)))));
    ...
    private static int getMessagePriority(String str) {
        ...
        return MixpanelAnalyticsManager.CARD_MODE_NORMAL.equals(str) ? 2 : 0;
    }
