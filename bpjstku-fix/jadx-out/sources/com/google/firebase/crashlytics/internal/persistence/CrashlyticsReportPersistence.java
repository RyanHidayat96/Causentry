...
public class CrashlyticsReportPersistence {
    ...
    public void persistEvent(CrashlyticsReport.Session.Event event, String str, boolean z) {
        ...
        try {
            ...
            Logger.getLogger().w("Could not persist event for session ".concat(String.valueOf(str)), e2);
        }
        trimEvents(str, i);
    }
    ...
    public void finalizeReports(String str, long j) {
        for (String str2 : capAndGetOpenSessions(str)) {
            Logger.getLogger().v("Finalizing report for session ".concat(String.valueOf(str2)));
    ...
    private SortedSet<String> capAndGetOpenSessions(String str) {
        ...
        if (openSessionIds.size() > 8) {
            while (openSessionIds.size() > 8) {
                ...
                Logger.getLogger().d("Removing session over cap: ".concat(String.valueOf(strLast)));
        ...
        loop0: while (true) {
            ...
            while (true) {
                ...
                try {
                    ...
                    Logger.getLogger().w("Could not add event to report for ".concat(String.valueOf(next)), e2);
                }
            }
        }
        if (arrayList.isEmpty()) {
            Logger.getLogger().w("Could not parse event files for session ".concat(String.valueOf(str)));
            return;
        }
    ...
    private void synthesizeReportFile(File file, List<CrashlyticsReport.Session.Event> list, long j, boolean z, String str, String str2) {
        ...
        try {
            ...
            Logger.getLogger().w("Could not synthesize final report file for ".concat(String.valueOf(file)), e2);
        }
    }
    ...
    private static String generateEventFilename(int i, boolean z) {
        String str = String.format(Locale.US, EVENT_COUNTER_FORMAT, Integer.valueOf(i));
        String str2 = z ? PRIORITY_EVENT_SUFFIX : "";
