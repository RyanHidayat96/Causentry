                ...
                if (!skipMonth(item, item2, Long.valueOf(jLongValue), Long.valueOf(jLongValue2))) {
                    ...
                    if (jLongValue < item.longValue()) {
                        if (adapter.isFirstInRow(iMax)) {
                            ...
                            left2 = this.getChildAtPosition(iMax - 1).getRight();
                        } else {
                            left2 = this.getChildAtPosition(iMax - 1).getLeft();
                        }
                        iHorizontalMidPoint = left2;
                        iDayToPosition = iMax;
                    } else {
                        this.dayCompute.setTimeInMillis(jLongValue);
                        iDayToPosition = adapter.dayToPosition(this.dayCompute.get(5));
                        iHorizontalMidPoint = horizontalMidPoint(this.getChildAtPosition(iDayToPosition));
                    }
                    if (jLongValue2 > item2.longValue()) {
                        if (adapter.isLastInRow(iMin)) {
                            left = getWidth();
                        } else if (!zIsLayoutRtl) {
                            left = this.getChildAtPosition(iMin).getRight();
                        } else {
                            left = this.getChildAtPosition(iMin).getLeft();
                        }
                        iHorizontalMidPoint2 = left;
                        iDayToPosition2 = iMin;
                    } else {
                        this.dayCompute.setTimeInMillis(jLongValue2);
                        iDayToPosition2 = adapter.dayToPosition(this.dayCompute.get(5));
                        iHorizontalMidPoint2 = horizontalMidPoint(this.getChildAtPosition(iDayToPosition2));
                    }
                    ...
                    while (itemId <= itemId2) {
                        ...
                        View childAtPosition = this.getChildAtPosition(numColumns);
