...
public class MaterialMainContainerBackHelper extends MaterialBackAnimationHelper<View> {
    ...
    public void updateBackProgress(float f, boolean z, float f2, float f3) {
        ...
        float fLerp2 = AnimationUtils.lerp(0.0f, Math.max(0.0f, ((width - (MIN_SCALE * width)) / 2.0f) - this.minEdgeGap), fInterpolateProgress);
        int i = z ? 1 : -1;
        ...
        float fAbs = Math.abs(f4) / height;
        float fSignum = Math.signum(f4);
        float fLerp3 = AnimationUtils.lerp(0.0f, fMin, fAbs);
        this.view.setScaleX(fLerp);
        this.view.setScaleY(fLerp);
        this.view.setTranslationX(fLerp2 * i);
        this.view.setTranslationY(fLerp3 * fSignum);
