...
public class ColorUtils {
    ...
    public static int argbFromXyz(double d, double d2, double d3) {
        ...
        double d9 = dArr3[2];
        double[] dArr4 = dArr[2];
        return argbFromRgb(delinearized((d4 * d) + (d5 * d2) + (d6 * d3)), delinearized((d7 * d) + (d8 * d2) + (d9 * d3)), delinearized((dArr4[0] * d) + (dArr4[1] * d2) + (dArr4[2] * d3)));
    }
    ...
    public static int argbFromLab(double d, double d2, double d3) {
        ...
        double dLabInvf = labInvf((d2 / 500.0d) + d4);
        double dLabInvf2 = labInvf(d4);
        double dLabInvf3 = labInvf(d4 - (d3 / 200.0d));
        return argbFromXyz(dLabInvf * dArr[0], dLabInvf2 * dArr[1], dLabInvf3 * dArr[2]);
    }
    ...
    public static double[] labFromArgb(int i) {
        ...
        double d10 = (((d * dLinearized) + (d2 * dLinearized2)) + (d3 * dLinearized3)) / dArr5[0];
        double d11 = (((d4 * dLinearized) + (d5 * dLinearized2)) + (d6 * dLinearized3)) / dArr5[1];
        double d12 = (((d7 * dLinearized) + (d8 * dLinearized2)) + (d9 * dLinearized3)) / dArr5[2];
        double dLabF = labF(d10);
        double dLabF2 = labF(d11);
        return new double[]{(116.0d * dLabF2) - 16.0d, (dLabF - dLabF2) * 500.0d, (dLabF2 - labF(d12)) * 200.0d};
    }
