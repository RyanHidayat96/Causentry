...
class KeylineStateList {
    ...
    private static List<KeylineState> getStateStepsStart(Carousel carousel, KeylineState keylineState, float f, float f2) {
        ...
        if (!isFirstFocalItemAtLeftOfContainer(keylineState)) {
            int i = -1;
            if (iFindFirstNonAnchorKeylineIndex != -1) {
                ...
                while (i2 < firstFocalKeylineIndex) {
                    ...
                    KeylineState keylineStateMoveKeylineAndCreateKeylineState = moveKeylineAndCreateKeylineState(keylineState2, iFindFirstNonAnchorKeylineIndex, i4 >= 0 ? findFirstIndexAfterLastFocalKeylineWithMask(keylineState2, keylineState.getKeylines().get(i4).mask) - 1 : size, f3 + f5, (keylineState.getFirstFocalKeylineIndex() - i2) - 1, (keylineState.getLastFocalKeylineIndex() - i2) - 1, containerWidth);
                    if (i5 == i6 - 1 && f2 > 0.0f) {
    ...
    Map<Integer, KeylineState> getKeylineStateForPositionMap(int i, int i2, int i3, boolean z) {
        ...
        while (true) {
            ...
            if (i6 * itemSize * (z ? -1 : 1) > i3 - this.endShiftRange || i4 >= i - this.endStateSteps.size()) {
                List<KeylineState> list = this.endStateSteps;
                map.put(Integer.valueOf(i6), list.get(MathUtils.clamp(i5, 0, list.size() - 1)));
                i5++;
            }
            i4++;
        }
        ...
        for (int i8 = i - 1; i8 >= 0; i8--) {
            ...
            if (i9 * itemSize * (z ? -1 : 1) < i2 + this.startShiftRange || i8 < this.startStateSteps.size()) {
                ...
                map.put(Integer.valueOf(i9), list2.get(MathUtils.clamp(i7, 0, list2.size() - 1)));
                i7++;
            }
        }
        return map;
