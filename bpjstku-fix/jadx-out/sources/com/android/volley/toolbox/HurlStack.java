...
public class HurlStack extends BaseHttpStack {
    ...
    public static int TuitionPaymentFragmentspecialinlinedviewModeldefault3 = 1830233424;
    ...
    public HttpResponse executeRequest(Request<?> request, Map<String, String> map) throws IOException, AuthFailureError {
        ...
        try {
            ...
            int iTuitionPaymentFragmentspecialinlinedviewModeldefault2 = ViewPortBuilder.TuitionPaymentFragmentspecialinlinedviewModeldefault2(httpURLConnectionOpenConnection);
            if (iTuitionPaymentFragmentspecialinlinedviewModeldefault2 == -1) {
                throw new IOException("Could not retrieve response code from HttpUrlConnection.");
            }
            if (!hasResponseBody(request.getMethod(), iTuitionPaymentFragmentspecialinlinedviewModeldefault2)) {
                HttpResponse httpResponse = new HttpResponse(iTuitionPaymentFragmentspecialinlinedviewModeldefault2, convertHeaders(httpURLConnectionOpenConnection.getHeaderFields()));
                httpURLConnectionOpenConnection.disconnect();
                return httpResponse;
            }
            return new HttpResponse(iTuitionPaymentFragmentspecialinlinedviewModeldefault2, convertHeaders(httpURLConnectionOpenConnection.getHeaderFields()), httpURLConnectionOpenConnection.getContentLength(), createInputStream(request, httpURLConnectionOpenConnection));
        } catch (Throwable th) {
            if (0 == 0) {
                httpURLConnectionOpenConnection.disconnect();
            }
    ...
    public static InputStream inputStreamFromConnection(HttpURLConnection httpURLConnection) {
        try {
            return ViewPortBuilder.TuitionPaymentFragmentspecialinlinedviewModeldefault3(httpURLConnection);
    ...
    protected OutputStream createOutputStream(Request<?> request, HttpURLConnection httpURLConnection, int i) throws IOException {
        return ViewPortBuilder.TuitionPaymentFragmentbindingInflater1(httpURLConnection);
    }
}
