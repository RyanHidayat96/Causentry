...
public class LicenseResponseHelper {
    ...
    public static void validateResponse(Bundle responsePayload, String packageName) throws LicenseCheckException {
        try {
            ...
            if (!jSONObjectBase64ToJson2.getString("packageName").equals(packageName)) {
    ...
    private static void verifySignature(String signedData, String signature, String signatureAlgorithm, PublicKey publicKey) throws LicenseCheckException {
        try {
            ...
            throw new LicenseCheckException(String.format("Could not find %s algorithm on the device", signatureAlgorithm), e4);
        } catch (SignatureException e5) {
            throw new LicenseCheckException("Could not parse returned signature.", e5);
        }
    }
    ...
    private static PublicKey getPublicKey() throws LicenseCheckException {
        try {
            ...
            throw new LicenseCheckException(String.format("%s algorithm not found on device", KEY_FACTORY_ALGORITHM), e3);
