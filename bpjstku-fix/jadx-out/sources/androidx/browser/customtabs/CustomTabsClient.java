...
public class CustomTabsClient {
    ...
    CustomTabsClient(MediaBrowserCompatConnectionCallbackConnectionCallbackInternal mediaBrowserCompatConnectionCallbackConnectionCallbackInternal, ComponentName componentName, Context context) {
    ...
    public static String getPackageName(Context context, List<String> list, boolean z) {
        ...
        if (!z && (resolveInfoResolveActivity = packageManager.resolveActivity(intent, 0)) != null) {
            String str = ((PackageItemInfo) resolveInfoResolveActivity.activityInfo).packageName;
    ...
    private CustomTabsSession newSessionInternal(CustomTabsCallback customTabsCallback, PendingIntent pendingIntent) {
        boolean zNewSession;
        onConnected.TuitionPaymentFragmentspecialinlinedviewModeldefault2 tuitionPaymentFragmentspecialinlinedviewModeldefault2CreateCallbackWrapper = createCallbackWrapper(customTabsCallback);
        try {
            if (pendingIntent != null) {
                Bundle bundle = new Bundle();
                bundle.putParcelable(CustomTabsIntent.EXTRA_SESSION_ID, pendingIntent);
                zNewSession = this.mService.newSessionWithExtras(tuitionPaymentFragmentspecialinlinedviewModeldefault2CreateCallbackWrapper, bundle);
            } else {
                zNewSession = this.mService.newSession(tuitionPaymentFragmentspecialinlinedviewModeldefault2CreateCallbackWrapper);
            }
            if (zNewSession) {
                return new CustomTabsSession(this.mService, tuitionPaymentFragmentspecialinlinedviewModeldefault2CreateCallbackWrapper, this.mServiceComponentName, pendingIntent);
            }
            return null;
        } catch (RemoteException unused) {
            return null;
    ...
    private onConnected.TuitionPaymentFragmentspecialinlinedviewModeldefault2 createCallbackWrapper(final CustomTabsCallback customTabsCallback) {
        return new onConnected.TuitionPaymentFragmentspecialinlinedviewModeldefault2() { // from class: androidx.browser.customtabs.CustomTabsClient.2
