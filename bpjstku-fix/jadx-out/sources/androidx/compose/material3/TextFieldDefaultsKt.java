...
public final class TextFieldDefaultsKt {
    ...
    public static final State<BorderStroke> m2643animateBorderStrokeAsStateNuRrP5Q(boolean z, boolean z2, InteractionSource interactionSource, TextFieldColors textFieldColors, float f, float f2, Composer composer, int i) {
        ...
        float f3 = !animateBorderStrokeAsState_NuRrP5Q$lambda$0(stateCollectIsFocusedAsState) ? f2 : f;
        if (z) {
            composer.startReplaceableGroup(-1927721478);
            stateRememberUpdatedState = AnimateAsStateKt.m449animateDpAsStateAjpBEmI(f3, AnimationSpecKt.tween$default(150, 0, null, 6, null), null, null, composer, 48, 12);
            composer.endReplaceableGroup();
        } else {
            composer.startReplaceableGroup(-1927721380);
            stateRememberUpdatedState = SnapshotStateKt.rememberUpdatedState(Dp.m6933boximpl(f2), composer, (i >> 15) & 14);
            composer.endReplaceableGroup();
        }
        State<BorderStroke> stateRememberUpdatedState2 = SnapshotStateKt.rememberUpdatedState(new BorderStroke(stateRememberUpdatedState.getValue().m6949unboximpl(), new SolidColor(stateIndicatorColor$material3_release.getValue().m4228unboximpl(), null), null), composer, 0);
        if (ComposerKt.isTraceInProgress()) {
