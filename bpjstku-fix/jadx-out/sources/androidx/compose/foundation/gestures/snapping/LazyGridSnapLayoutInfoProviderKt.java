...
public final class LazyGridSnapLayoutInfoProviderKt {
    ...
    public static final SnapLayoutInfoProvider SnapLayoutInfoProvider(final LazyGridState lazyGridState, final SnapPosition snapPosition) {
        return new SnapLayoutInfoProvider() { // from class: androidx.compose.foundation.gestures.snapping.LazyGridSnapLayoutInfoProviderKt.SnapLayoutInfoProvider.1
            ...
            public final float calculateApproachOffset(float p0, float p1) {
                return RangesKt.coerceAtLeast(Math.abs(p1) - getAverageItemSize(), 0.0f) * Math.signum(p1);
            }
            ...
            public final float calculateSnapOffset(float p0) {
                ...
                return SnapFlingBehaviorKt.m848calculateFinalOffsetFhqu1e0(LazyListSnapLayoutInfoProviderKt.calculateFinalSnappingItem(lazyGridState.getDensity$foundation_release(), p0), f, f2);
            }
        };
    }
    ...
    public static final int getSingleAxisViewportSize(LazyGridLayoutInfo lazyGridLayoutInfo) {
        if (lazyGridLayoutInfo.getOrientation() == Orientation.Vertical) {
            return IntSize.m7105getHeightimpl(lazyGridLayoutInfo.mo1142getViewportSizeYbymL2g());
        }
        return IntSize.m7106getWidthimpl(lazyGridLayoutInfo.mo1142getViewportSizeYbymL2g());
    }

    public static final int sizeOnMainAxis(LazyGridItemInfo lazyGridItemInfo, Orientation orientation) {
        if (orientation == Orientation.Vertical) {
            return IntSize.m7105getHeightimpl(lazyGridItemInfo.getSize());
        }
        return IntSize.m7106getWidthimpl(lazyGridItemInfo.getSize());
    }

    public static final int offsetOnMainAxis(LazyGridItemInfo lazyGridItemInfo, Orientation orientation) {
        if (orientation == Orientation.Vertical) {
            return IntOffset.m7064getYimpl(lazyGridItemInfo.getOffset());
        }
        return IntOffset.m7063getXimpl(lazyGridItemInfo.getOffset());
    }
}
