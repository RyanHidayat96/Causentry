...
final class UnionInsets implements WindowInsets {
    ...
    public final int getLeft(Density p0, LayoutDirection p1) {
        return Math.max(this.first.getLeft(p0, p1), this.second.getLeft(p0, p1));
    }

    @Override // androidx.compose.foundation.layout.WindowInsets
    public final int getTop(Density p0) {
        return Math.max(this.first.getTop(p0), this.second.getTop(p0));
    }

    @Override // androidx.compose.foundation.layout.WindowInsets
    public final int getRight(Density p0, LayoutDirection p1) {
        return Math.max(this.first.getRight(p0, p1), this.second.getRight(p0, p1));
    }

    @Override // androidx.compose.foundation.layout.WindowInsets
    public final int getBottom(Density p0) {
        return Math.max(this.first.getBottom(p0), this.second.getBottom(p0));
    }
    ...
    public final boolean equals(Object p0) {
        ...
        if (!(p0 instanceof UnionInsets)) {
            return false;
        }
        UnionInsets unionInsets = (UnionInsets) p0;
