...
public final class ColorSpaces {
    ...
    public final ColorSpace match(float[] p0, TransferParameters p1) {
        for (ColorSpace colorSpace : ColorSpacesArray) {
            if (ColorModel.m4644equalsimpl0(colorSpace.getModel(), ColorModel.INSTANCE.m4651getRgbxdoWZVw())) {
                ...
                Intrinsics.checkNotNull(colorSpaceAdapt$default, "");
                Rgb rgb = (Rgb) colorSpaceAdapt$default;
                if (ColorSpaceKt.compare(p0, rgb.getTransform()) && ColorSpaceKt.compare(p1, rgb.getTransferParameters())) {
                    return colorSpace;
                }
            }
        }
    ...
    public final ColorSpace getColorSpace$ui_graphics_release(int p0) {
        return getColorSpacesArray$ui_graphics_release()[p0];
    }
    ...
    public final double transferHlgOetf$ui_graphics_release(TransferParameters p0, double p1) {
        double dLog;
        double d = p1 < 0.0d ? -1.0d : 1.0d;
        double a2 = 1.0d / p0.getA();
        double b = 1.0d / p0.getB();
        double c = 1.0d / p0.getC();
        double d2 = p0.getD();
        double e2 = p0.getE();
        double d3 = d;
        double f = (p1 * d) / (p0.getF() + 1.0d);
        if (f <= 1.0d) {
            dLog = a2 * Math.pow(f, b);
        } else {
            dLog = (c * Math.log(f - d2)) + e2;
        }
        return d3 * dLog;
    }

    public final double transferHlgEotf$ui_graphics_release(TransferParameters p0, double p1) {
        double dExp;
        double d = p1 < 0.0d ? -1.0d : 1.0d;
        double d2 = p1 * d;
        double a2 = p0.getA();
        ...
        if (d4 <= 1.0d) {
            ...
            dExp = Math.exp((d2 - e2) * c) + d3;
        }
        return (f + 1.0d) * d * dExp;
    }

    public final double transferSt2048Oetf$ui_graphics_release(TransferParameters p0, double p1) {
        double d = p1 < 0.0d ? -1.0d : 1.0d;
        double d2 = p1 * d;
        double d3 = -p0.getA();
        double d4 = p0.getD();
        double f = 1.0d / p0.getF();
        return d * Math.pow(Math.max(d3 + (d4 * Math.pow(d2, f)), 0.0d) / (p0.getB() + ((-p0.getE()) * Math.pow(d2, f))), 1.0d / p0.getC());
    }

    public final double transferSt2048Eotf$ui_graphics_release(TransferParameters p0, double p1) {
        double d = p1 < 0.0d ? -1.0d : 1.0d;
        double d2 = p1 * d;
        return d * Math.pow(RangesKt.coerceAtLeast(p0.getA() + (p0.getB() * Math.pow(d2, p0.getC())), 0.0d) / (p0.getD() + (p0.getE() * Math.pow(d2, p0.getC()))), p0.getF());
    }
}
