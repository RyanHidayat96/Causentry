...
public final class SetComposingTextCommand implements EditCommand {
    ...
    public final void applyTo(EditingBuffer p0) {
        if (p0.hasComposition$ui_text()) {
            int compositionStart = p0.getCompositionStart();
            p0.replace$ui_text(p0.getCompositionStart(), p0.getCompositionEnd(), getText());
            if (getText().length() > 0) {
                p0.setComposition$ui_text(compositionStart, getText().length() + compositionStart);
            }
        } else {
            int selectionStart = p0.getSelectionStart();
            p0.replace$ui_text(p0.getSelectionStart(), p0.getSelectionEnd(), getText());
            if (getText().length() > 0) {
                p0.setComposition$ui_text(selectionStart, getText().length() + selectionStart);
            }
        }
    ...
    public final boolean equals(Object p0) {
        ...
        if (!(p0 instanceof SetComposingTextCommand)) {
            return false;
        }
        SetComposingTextCommand setComposingTextCommand = (SetComposingTextCommand) p0;
