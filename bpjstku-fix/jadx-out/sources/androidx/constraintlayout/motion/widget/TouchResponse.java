...
class TouchResponse {
    ...
    /* JADX WARN: Code duplicated, block: B:58:0x028e  */
    /* JADX WARN: Code duplicated, block: B:61:0x02ab  */
    /* JADX WARN: Code duplicated, block: B:62:0x02b7  */
            ...
            if (action == 2) {
                ...
                if (Math.abs(fAtan2) > 0.01d || this.mDragStarted) {
                    ...
                    if (fMax != progress2) {
                        ...
                        double d = xVelocity;
                        this.mMotionLayout.mLastVelocity = (float) Math.toDegrees((float) ((Math.hypot(yVelocity, d) * Math.sin(Math.atan2(yVelocity, d) - dAtan2)) / Math.hypot(rawX - width, rawY - height)));
                    } else {
