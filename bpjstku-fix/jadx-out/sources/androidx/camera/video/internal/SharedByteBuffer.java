...
public final class SharedByteBuffer implements Closeable {
    ...
    private SharedByteBuffer(ByteBuffer byteBuffer, AtomicInteger atomicInteger, Pair<Executor, Runnable> pair, int i) {
        ...
        this.mShareId = i;
        if (Logger.isDebugEnabled(TAG) && (i2 = atomicInteger.get()) <= 0) {
        ...
        synchronized (this.mCloseLock) {
            ...
            if (iDecrementAndGet == 0) {
                ...
                try {
                    ...
                    Logger.e(TAG, String.format(Locale.US, "Unable to execute final close action. [%s]", toString()), e2);
                }
            }
            return true;
        }
