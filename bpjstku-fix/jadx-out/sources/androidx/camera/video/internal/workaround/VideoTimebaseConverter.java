...
public class VideoTimebaseConverter {
    ...
    public long convertToUptimeUs(long j) {
        ...
        if (i != 1) {
            ...
            StringBuilder sb = new StringBuilder("Unknown timebase: ");
            sb.append(this.mResolvedInputTimebase);
            throw new AssertionError(sb.toString());
        }
        if (this.mUptimeToRealtimeOffsetUs == -1) {
            this.mUptimeToRealtimeOffsetUs = calculateUptimeToRealtimeOffsetUs();
            StringBuilder sb2 = new StringBuilder("mUptimeToRealtimeOffsetUs = ");
            sb2.append(this.mUptimeToRealtimeOffsetUs);
    ...
    private Timebase resolveInputTimebase(long j) {
        ...
        if (z && timebase != this.mInputTimebase) {
            ...
            Logger.e(TAG, String.format("Detected camera timebase inconsistent. Please file an issue at https://issuetracker.google.com/issues/new?component=618491&template=1257717 with this error message [Manufacturer: %s, Model: %s, Hardware: %s, API Level: %d%s].\nCamera timebase is inconsistent. The timebase reported by the camera is %s, but the actual timebase contained in the frame is detected as %s.", str, str2, str3, Integer.valueOf(Build.VERSION.SDK_INT), string, this.mInputTimebase, timebase));
            return timebase;
        }
        Logger.d(TAG, "Detect input timebase = ".concat(String.valueOf(timebase)));
        return timebase;
    }
