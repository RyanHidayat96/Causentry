...
final class PreviewTransformation {
    ...
    final void setTransformationInfo(SurfaceRequest.TransformationInfo transformationInfo, Size size, boolean z) {
    ...
    final void overrideWithDisplayRotation(int i, int i2) {
    ...
    final Matrix getTextureViewCorrectionMatrix() {
    ...
    final void transformView(Size size, int i, View view) {
        if (size.getHeight() == 0 || size.getWidth() == 0) {
            Logger.w(TAG, "Transform not applied due to PreviewView size: ".concat(String.valueOf(size)));
            return;
        }
    ...
    final void setScaleType(PreviewView.ScaleType scaleType) {
        this.mScaleType = scaleType;
    }

    final PreviewView.ScaleType getScaleType() {
    ...
    final Matrix getSensorToViewTransform(Size size, int i) {
        if (!isTransformationInfoReady()) {
            return null;
        }
    ...
    final Matrix getSurfaceToPreviewViewMatrix(Size size, int i) {
    ...
    final RectF getPreviewViewViewportRectForMismatchedAspectRatios(Size size, int i) {
    ...
    private static void setMatrixRectToRect(Matrix matrix, RectF rectF, RectF rectF2, PreviewView.ScaleType scaleType) {
        ...
        switch (AnonymousClass1.$SwitchMap$androidx$camera$view$PreviewView$ScaleType[scaleType.ordinal()]) {
            ...
            case 6:
                ...
                break;
            default:
                Logger.e(TAG, "Unexpected crop rect: ".concat(String.valueOf(scaleType)));
    ...
    final boolean isViewportAspectRatioMatchPreviewView(Size size) {
        return TransformUtils.isAspectRatioMatchingWithRoundingError(size, true, getRotatedViewportSize(), false);
    }

    final Rect getSurfaceCropRect() {
        return this.mSurfaceCropRect;
    }

    final Bitmap createTransformedBitmap(Bitmap bitmap, Size size, int i) {
        if (!isTransformationInfoReady()) {
            return bitmap;
        }
    ...
    final Matrix getPreviewViewToNormalizedSensorMatrix(Size size, int i, Rect rect) {
        if (!isTransformationInfoReady()) {
            return null;
        }
        Matrix matrix = new Matrix();
