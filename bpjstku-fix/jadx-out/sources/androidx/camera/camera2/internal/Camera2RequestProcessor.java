...
public class Camera2RequestProcessor implements RequestProcessor {
    ...
    public Camera2RequestProcessor(CaptureSession captureSession, List<SessionProcessorSurface> list) {
        ...
        StringBuilder sb = new StringBuilder("CaptureSession state must be OPENED. Current state:");
        sb.append(captureSession.mState);
    ...
    private boolean isRequestValid(RequestProcessor.Request request) {
        ...
        for (Integer num : request.getTargetOutputConfigIds()) {
            if (findSurface(num.intValue()) == null) {
                StringBuilder sb = new StringBuilder("Unable to submit the RequestProcessor.Request: targetOutputConfigId(");
                sb.append(num);
    ...
    class Camera2CallbackWrapper extends CameraCaptureSession.CaptureCallback {
