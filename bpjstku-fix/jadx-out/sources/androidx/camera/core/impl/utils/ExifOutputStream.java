...
public final class ExifOutputStream extends FilterOutputStream {
    ...
    private void writeExifSegment(ByteOrderedDataOutputStream byteOrderedDataOutputStream) throws IOException {
        ...
        byteOrderedDataOutputStream.writeUnsignedShort(size2 + 8);
        ...
        for (int i5 = 0; i5 < ExifData.EXIF_TAGS.length; i5++) {
            if (!this.mExifData.getAttributes(i5).isEmpty()) {
                byteOrderedDataOutputStream.writeUnsignedShort(this.mExifData.getAttributes(i5).size());
                int size3 = iArr[i5] + 2 + (this.mExifData.getAttributes(i5).size() * 12) + 4;
                for (Map.Entry<String, ExifAttribute> entry : this.mExifData.getAttributes(i5).entrySet()) {
                    ExifTag exifTag2 = ExifData.Builder.sExifTagMapsForWriting.get(i5).get(entry.getKey());
                    StringBuilder sb = new StringBuilder("Tag not supported: ");
                    sb.append(entry.getKey());
                    sb.append(". Tag needs to be ported from ExifInterface to ExifData.");
                    int i6 = ((ExifTag) Preconditions.checkNotNull(exifTag2, sb.toString())).number;
                ...
                Iterator<Map.Entry<String, ExifAttribute>> it2 = this.mExifData.getAttributes(i5).entrySet().iterator();
                while (it2.hasNext()) {
