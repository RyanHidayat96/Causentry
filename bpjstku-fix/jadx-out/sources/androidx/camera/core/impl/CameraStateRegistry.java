...
public final class CameraStateRegistry implements CameraCoordinator.ConcurrentCameraModeListener {
    ...
    public final void registerCamera(Camera camera, Executor executor, OnConfigureAvailableListener onConfigureAvailableListener, OnOpenAvailableListener onOpenAvailableListener) {
        synchronized (this.mLock) {
            boolean zContainsKey = this.mCameraStates.containsKey(camera);
            StringBuilder sb = new StringBuilder("Camera is already registered: ");
            sb.append(camera);
    ...
    public final boolean tryOpenCamera(Camera camera) {
        boolean z;
        synchronized (this.mLock) {
    ...
    public final boolean tryOpenCaptureSession(String str, String str2) {
        synchronized (this.mLock) {
            ...
            return (CameraInternal.State.OPEN.equals(state) || CameraInternal.State.CONFIGURED.equals(state)) && (CameraInternal.State.OPEN.equals(state2) || CameraInternal.State.CONFIGURED.equals(state2));
        }
    }

    public final void markCameraState(Camera camera, CameraInternal.State state) {
        markCameraState(camera, state, true);
    }

    /* JADX WARN: Code duplicated, block: B:18:0x003b  */
    public final void markCameraState(Camera camera, CameraInternal.State state, boolean z) {
        ...
        synchronized (this.mLock) {
            ...
            if (i <= 0 && this.mAvailableCameras > 0) {
                map = new HashMap();
    ...
    public final void onCameraOperatingModeUpdated(int i, int i2) {
        synchronized (this.mLock) {
            boolean z = true;
    ...
    public final boolean isCameraClosing() {
        synchronized (this.mLock) {
    ...
    static class CameraRegistration {
    ...
    private static void traceState(Camera camera, CameraInternal.State state) {
        if (Trace.isEnabled()) {
            StringBuilder sb = new StringBuilder("CX:State[");
            sb.append(camera);
            sb.append("]");
            Trace.setCounter(sb.toString(), state.ordinal());
        }
    }
}
