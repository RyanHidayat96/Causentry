...
public class SupportedOutputSizesSorter {
    ...
    private List<Size> getResolutionCandidateList(List<Pair<Integer, Size[]>> list, int i) {
        ...
        if (arrayList.isEmpty()) {
            StringBuilder sb = new StringBuilder("The retrieved supported resolutions from camera info internal is empty. Format is ");
            sb.append(i);
            sb.append(".");
    ...
    static Rational getTargetAspectRatioRationalValue(int i, boolean z) {
        ...
        if (i != 1) {
            Logger.e(TAG, "Undefined target aspect ratio: ".concat(String.valueOf(i)));
            return null;
        }
        if (z) {
