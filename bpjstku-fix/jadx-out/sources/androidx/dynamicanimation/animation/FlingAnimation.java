...
public final class FlingAnimation extends DynamicAnimation<FlingAnimation> {
    ...
    static final class DragForce implements Force {
        ...
        final void setFrictionScalar(float f) {
            this.mFriction = f * DEFAULT_FRICTION;
        }

        final float getFrictionScalar() {
            return this.mFriction / DEFAULT_FRICTION;
        }

        final DynamicAnimation.MassState updateValueAndVelocity(float f, float f2, long j) {
            float f3 = j;
            this.mMassState.mVelocity = (float) (((double) f2) * Math.exp((f3 / 1000.0f) * this.mFriction));
            DynamicAnimation.MassState massState = this.mMassState;
            float f4 = this.mFriction;
            float f5 = f2 / f4;
            massState.mValue = (float) (((double) (f - f5)) + (((double) f5) * Math.exp((f4 * f3) / 1000.0f)));
