...
# (No post-fs-data.sh on purpose: CODEX.md keeps that stage absent by default.)
MODDIR=${0%/*}
...
for f in lib.sh apply.sh restore.sh causentryd.sh root-apps.sh enforce-denylist.sh status.sh doctor.sh; do
